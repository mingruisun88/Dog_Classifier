...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    with open(dogfile, "r") as infile:
        ...
        while line != "":
            key = line.rstrip()
            if key not in dognames_dic.keys():
                dognames_dic.update({index:key})
                index+=1
            else:
                print('Warning:Dog name already in dognames_dic')
            line = infile.readline()
        infile.close()
        #print(dognames_dic)
    ...
    for key in results_dic:
        list_buff = results_dic.get(key)
        image_label = list_buff[0]
        classify_label = list_buff[1]
        if image_label in dognames_dic.values():
            list_buff.append(1)
        else:
            list_buff.append(0)
        if classify_label in dognames_dic.values():
            list_buff.append(1)
        else:
            list_buff.append(0)
        results_dic.update({key:list_buff})
